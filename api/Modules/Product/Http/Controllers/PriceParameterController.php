...
class PriceParameterController extends Controller
{
    ...
    {
        return response(PriceParameter::where('deleted', 0)->get()->toTree());
    }
    ...
    {
        return response([
            'item' => PriceParameter::select('id', 'title')->find($id),
            'list' => PriceParameter::where('id', '<>' , $id)->get(),
        ]);
    }
        ...
        if (!empty($selected)) {
            foreach ($selected as $item) {
                foreach ($new_node as $new) {
                    $node = new PriceParameter([
                        'title' => trim($new),
                    ]);
                    $node->appendToNode(PriceParameter::find($item));
                    $node->save();
                }
            }
    ...
    public function update($id, Request $request)
    {
        ...
        $nodes = PriceParameter::descendantsAndSelf($id);
